...
Screen.prototype._handleRawUpdate = function(rect) {
  console.log('GOT UPDATE', rect.width, rect.height);//, rect);
  if (rect.encoding ===  rfb.encodings.raw) {
    ...
    if (r.width > 30 && r.height > 30) {
      this._screenBuffer.save(`screen-${counter}-full.png`)
      counter++;
    }
...
Screen.prototype.mouseButtonDown = function(button) {
...
Screen.prototype.mouseButtonUp = function(button) {
  const buttonMask = 1 << button;
  this._mouseState.buttons = this._mouseState.buttons & (~buttonMask)
  this._rfbConn.pointerEvent(
    this._mouseState.x,
