...
function createScreenDriver(opts) {
  // TODO
}

function Screen(opts) {
  EventEmitter.call(this);
  this._screenDriver = createScreenDriver(opts);
  ...
  this._screenDriver.on('connect', () => {
    this.emit('connect');
  });
  this._screenDriver.on('error', e => {
    this.emit('error', e);
  });
}
...
Screen.prototype._connected = async function() {
  if (this._screenDriver && this._screenDriver.getContent()) {
...
Screen.prototype.mouseButtonDown = async function(button) {
  ...
  this._screenDriver.pointerEvent(this._mouseState.x, this._mouseState.y, this._mouseState.buttons);
};
...
Screen.prototype.mouseMove = async function(x, y) {
  ...
  return new Promise((accept, reject) => {
    const updatePosition = () => {
      ...
      if (dt < eta) {
        this._screenDriver.pointerEvent(x, y, this._mouseState.buttons);
        this._mouseState.x = x;
        this._mouseState.y = y;
        return setTimeout(updatePosition, moveInterval);
      } else {
        this._screenDriver.pointerEvent(location.x, location.y, this._mouseState.buttons);
...
Screen.prototype.keyboardKeyDown = async function(key) {
  ...
  this._screenDriver.keyEvent(keySym, 1);
};

Screen.prototype.keyboardKeyUp = async function(key) {
  await this._connected();
  const keySym = typeof key === 'string' ? keysym.fromName(key).keysym : key;
  this._screenDriver.keyEvent(keySym, 0);
};
