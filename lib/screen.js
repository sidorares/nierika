...
Screen.prototype.createTemplateFromFile = async function(path) {
  return new Promise((accept, reject) => {
    ...
    cv.readImage(path, (err, img) => {
      ...
      const templateNoAlpha = rgb(template)
      accept(new Template({matrix: templateNoAlpha}))
    })
  })
}
...
Screen.prototype.waitVisible = async function(template, maxWait=0) {
  ...
  return new Promise((accept, reject) => {
    ...
    const findMatch = (rect) => {
      ...
      if (rect) {
        console.log('findMatch', rect.x, rect.y, rect.width, rect.height)
        try {
          ...
          //console.log('Rect:', rect.x, rect.y, rect.width, rect.height, 'Roi:', left, right, top, bottom, right - left, bottom - top, 'region:', region.width(), region.height())
          console.log('roi: ', left, top, right - left, bottom - top)
      ...
      region.save(`screen-${counter}.png`)
      ...
      if (m[0]) {
        console.log('GOT MATCH!!!!', counter, m)
